...
const ImageGridItem = ({ src = 'https://images.unsplash.com/photo-1761839256547-0a1cd11b6dfb', alt = 'shoot-image' }: ImageGridItemProps) => {
  ...
  return (
    <>
      <div className='relative w-full aspect-square rounded-lg overflow-hidden bg-foreground'>
        {/* Image placeholder - black rectangle */}
        <Image src={src} alt={alt} width={100} height={100} className='w-full h-full object-cover border-2 rounded-lg border-foreground' />

        
        {/* White bar overlay at bottom with rounded ends */}
        <div className='absolute bottom-2 inset-x-2 h-5.5 flex gap-1'>
          <Button 
            onClick={handleViewClick}
            className='bg-background border border-foreground text-foreground flex-centerize flex-1 h-full p-1! text-xs'
          >
            View
          </Button>
          <Button className='bg-background border border-foreground flex-centerize w-5.5 '>
